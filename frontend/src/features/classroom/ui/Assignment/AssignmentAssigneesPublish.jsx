...
export default function AssignmentAssigneesPublish({ classroomId, overview }) {
    ...
    const saveAssignment = async (publishFlag = false) => {
        if (!assignment || !assignment._id) {
            throw new Error("Assignment missing. Please start from step 1.");
        }
        if (assigneeType === "selected" && (!selected || selected.length === 0)) {
            throw new Error("Please choose at least one assignee.");
        }
        ...
        const payload = {
            ...
            publish: publishFlag,
            classesAssigned:
                assignment?.classesAssigned?.length
                    ? assignment.classesAssigned
                    : classroomId
                        ? [classroomId]
                        : [],
        };
    ...
    return (
        <div className="rounded-2xl border border-[#efe7ff] bg-white p-6 max-w-3xl mx-auto shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <p className="text-2xl font-semibold text-[#0b1220]">Assign & Publish</p>
                    <p className="text-sm text-slate-500 mt-1">
                        Choose who should receive this assignment and save when ready.
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <label className="inline-flex items-center gap-2 text-sm select-none">
                        <input
                            ...
                            onChange={(e) => setPublishChecked(Boolean(e.target.checked))}
                            className="h-4 w-4"
                        />
                    ...
                    <div className={`hidden sm:flex items-center`}>
                        <div
                            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${publishChecked
                                ? "bg-[#e6fff2] text-[#057a42] border border-[#dff5e6]"
                                : "bg-[#fff7e6] text-[#6b4d00] border border-[#fff0d6]"
                                }`}
                        >
            ...
            <div className="mt-6 space-y-6">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    type="button"
                                    onClick={selectAll}
                                    className="text-sm px-3 py-1 rounded-md border border-[#f1f1f16c] hover:bg-[#f7f7f75b]"
                                >
                                    Select all
                                </button>
                                <button
                        ...
                        <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-56 overflow-auto pr-2 my-4">
                            ...
                            {filteredMembers.map((m) => {
                                ...
                                return (
                                    <label
                                        key={uid}
                                        className={`flex items-center gap-3 rounded-2xl p-3 shadow-sm hover:shadow-md transition-all cursor-pointer select-none ${selected.includes(uid)
                                            ? "border border-[#5c2bc0] bg-[#7028c225]"
                ...
                <div className="flex justify-end gap-3 pt-2">
                    <Button variant="light" onClick={goBack} className="bg-white text-black">
                        Back
                    </Button>
                    ...
                    <Button
                        ...
                        variant="light"
                        className="bg-white/95 text-[#0b1220]"
                        disabled={saving}
                    >
