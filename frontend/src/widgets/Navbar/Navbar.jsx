...
const NavItem = React.memo(({ link, pathname, openDropdown, closeDropdown, centerDropdownOpen }) => {
  ...
  return (
    <div className={`flex-none group relative px-3 cursor-default text-center`} onMouseEnter={() => hasMenu && openDropdown(link.title)} onMouseLeave={() => hasMenu && closeDropdown(link.title)}>
      {hasMenu ? (
        <button
          type="button"
          aria-expanded={centerDropdownOpen === link.title}
          onClick={() => openDropdown(link.title)}
        ...
        <Link to={link.path} className={`relative block transition duration-300 px-3 py-2.5 ${active ? "text-violet-600" : "hover:text-violet-600"}`}>{link.title}</Link>
      )}
      ...
      {hasMenu && <Dropdown anchorTitle={link.title} source={link.subLinks || link.getSubLinks} open={centerDropdownOpen === link.title} />}
    </div>
  );
});

const Dropdown = ({ anchorTitle, source, open }) => {
  ...
  return (
    <div
      ...
      {loading ? (
        ...
        items.map((s, i) => (
          <Link key={i} to={typeof s === "string" ? `/${s}` : `/catalog/${(s.name || s.title || s.id || "").toString().split(" ").join("-").toLowerCase()}`} className="relative block whitespace-nowrap px-3 py-2.5 text-sm rounded-lg hover:bg-violet-50/40 hover:text-violet-600 transition-colors" onClick={() => { }}>
...
const ProfileMenu = ({ user, onSignOut }) => {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((s) => !s)} aria-expanded={open} aria-haspopup="true" className="flex items-center gap-2 px-3 py-1.5 rounded-full hover:bg-violet-50 transition-colors duration-200 focus:outline-none">
        <Avatar user={user} />
...
const MobileNav = ({ links, open, onClose, pathname, centerDropdownOpen, setCenterDropdownOpen }) => {
  const mobileClose = useCallback(() => onClose(), [onClose]);
  return (
    <nav aria-label="Main menu" className={`xl:hidden transition-transform duration-300 ease-in-out text-sm fixed left-0 top-0 bottom-0 z-50 bg-white shadow-[1px_0_rgba(86,75,128,0.1)] overflow-hidden ${open ? "translate-x-0" : "-translate-x-full"} w-[320px] sm:w-80`}>
      <div className="flex items-center justify-between px-6 py-4">
        <div className="w-[36px]" aria-hidden="true" />
        <button onClick={mobileClose} aria-label="Close menu" className="w-9 h-9 rounded-full flex items-center justify-center bg-white border border-gray-100 shadow-sm text-violet-600 hover:bg-violet-50 transition">
              ...
              {hasMenu ? (
                <div>
                  <div className="flex items-center justify-between py-3">
                    ...
                    <button className="text-lg leading-none px-2 py-1 text-gray-500" onClick={() => setCenterDropdownOpen((s) => (s === link.title ? null : link.title))} aria-expanded={centerDropdownOpen === link.title}>{centerDropdownOpen === link.title ? "−" : "+"}</button>
                  </div>

                  {centerDropdownOpen === link.title && (
                    <div className="pl-4">
                      <MobileSubmenu source={link.subLinks || link.getSubLinks} onClose={mobileClose} />
                    </div>
                  )}
                </div>
              ) : (
                <Link to={link.path} className={`block text-[15px] leading-7 py-3 transition-colors ${active ? "text-violet-600" : "text-gray-900 hover:text-violet-600"}`} onClick={mobileClose}>{link.title}</Link>
              )}
            </div>
          );
        })}
      ...
      <div className={open ? "fixed bottom-0 left-0 right-0 z-50 px-6 py-4 bg-white border-t border-gray-100" : "hidden"}>
        {/* keep mobile auth area similar to original */}
      </div>
    </nav>
  );
};

const MobileSubmenu = ({ source, onClose }) => {
  const { items, loading } = useSublinks(source);
  if (loading) return <div className="py-2 text-sm text-gray-600">Loading...</div>;
  if (!items?.length) return <div className="py-2 text-sm text-gray-600">No items</div>;

  return (
    <div>
      {items.map((s, idx) => (
        <Link key={idx} to={typeof s === "string" ? `/${s}` : `/catalog/${(s.name || s.title || s.id || "").toString().split(" ").join("-").toLowerCase()}`} className="block text-[15px] leading-7 py-2 hover:text-violet-600 text-gray-800" onClick={onClose}>{typeof s === "string" ? s : s.name || s.title}</Link>
      ))}
    </div>
  );
};
...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[500] w-full bg-transparent">
            ...
            <div className="ml-1 flex-none flex items-center rounded-full bg-white/75 bg-gradient-to-r from-pink-200/40 via-violet-200/40 to-indigo-200/40 border border-white/50 text-sm font-medium text-gray-800 shadow-lg shadow-gray-800/5 ring-1 ring-gray-800/[.075] backdrop-blur-xl px-0">
              {!token ? (
                <div className="flex items-center">
                  <Link to="/login" className="ml-1 mt-1 mb-1 flex-none group relative inline-flex items-center bg-clip-padding rounded-l-[20px] rounded-r-[8px] border h-8 pl-3 pr-3 bg-white/40 border-white/90 shadow hover:text-violet-600 hover:bg-violet-50/40 transition-colors duration-300"><span className="text-sm leading-none">Sign In</span></Link>
                  <Link to="/signup" className="mr-1 mt-1 mb-1 btn-purple flex-none group relative inline-flex items-center ml-1 h-8 pr-3 pl-3 rounded-r-[20px] rounded-l-[8px] bg-violet-600 hover:bg-violet-700 text-white text-sm transition-colors">Get Started</Link>
                </div>
              ) : (
      ...
      <div className={`xl:hidden fixed inset-0 z-40 transition-opacity duration-300 ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"} bg-gray-500/25 backdrop-blur-sm`} onClick={() => setMobileOpen(false)} aria-hidden={!mobileOpen} />

      <MobileNav links={NavbarLinks} open={mobileOpen} onClose={() => setMobileOpen(false)} pathname={pathname} centerDropdownOpen={centerDropdownOpen} setCenterDropdownOpen={setCenterDropdownOpen} />
    </header>
  );
};
