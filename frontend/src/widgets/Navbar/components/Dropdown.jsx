...
const Dropdown = ({ anchorTitle, path, source, open, closeDropdown }) => {
    const { items, loading } = useSublinks(source);

    return (
        <div
            className={`bubble-b tail absolute left-1/2 -translate-x-1/2 mt-2 z-50 w-48 rounded-2xl bg-white p-2 text-navy shadow-lg transition-all duration-200 transform origin-top ${open
                    ? "opacity-100 scale-100 translate-y-0 visible pointer-events-auto"
                    : "opacity-0 scale-95 -translate-y-2 invisible pointer-events-none"
                }`}
            onMouseLeave={() => closeDropdown(anchorTitle)}
        >
            ...
            {loading ? (
                ...
                items.map((s, i) => {
                    const url =
                        typeof s === "string"
                            ? `/${s}`
                            : `${path.toString().split(" ").join("-").toLowerCase()}/${(s.id || "")
                                .toString()
                                .split(" ")
                                .join("-")
                                .toLowerCase()}`;
