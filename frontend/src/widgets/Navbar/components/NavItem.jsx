...
const NavItem = React.memo(({ link, pathname, openDropdown, closeDropdown, centerDropdownOpen }) => {
    ...
    return (
        <div
            ...
            onMouseEnter={() => hasMenu && openDropdown(link.title)}
            onMouseLeave={() => hasMenu && closeDropdown(link.title)}
        >
            {hasMenu ? (
                <button
                    type="button"
                ...
                <Link to={link.path} className={`relative block transition duration-300 px-3 py-2.5 ${active ? "text-violet-600" : "hover:text-violet-600"}`}>
                    {link.title}
                </Link>
            )}
            ...
            {hasMenu && <Dropdown anchorTitle={link.title} path={link.path} source={link.subLinks || link.getSubLinks} open={centerDropdownOpen === link.title} />}
        </div>
    );
});
